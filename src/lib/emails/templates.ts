...
export function renderEmail(
  ...
  switch (templateId) {
    ...
    case 'newsletter_product_update': {
      headline = val(merge, 'newsletterHeadline', 'New arrivals &amp; specials')
      const intro = val(merge, 'introText', 'Check out the latest products and exclusive offers from Purpose Technology.')
      const productName = val(merge, 'featuredProductName', '')
      const productDesc = val(merge, 'featuredProductDesc', '')
      const productPrice = merge.featuredProductPrice?.trim() ? val(merge, 'featuredProductPrice', '') : ''
      const productImage = merge.featuredProductImage?.trim() ? merge.featuredProductImage.trim() : ''
        ...
        ? `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin:0 0 4px;">
            <tr>
              <td style="text-align:center; padding:0 0 16px;">
                <img src="${escapeHtml(productImage)}" alt="${productName || 'Featured product'}"
                  width="520" style="max-width:100%; height:auto; border-radius:16px; display:block; margin:0 auto; object-fit:cover;" />
              </td>
            </tr>
          </table>`
        : ''
      ...
      innerHtml =
        badge('Newsletter', '#1a72f0', '#eff6ff') +
        paragraph(intro) +
        (productName
          ? productImageBlock +
            callout(
              `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">` +
              `${detailRow('Featured', `<strong style="font-size:15px;">${productName}</strong>`)}` +
              (productDesc ? detailRow('About', productDesc) : '') +
              (productPrice ? detailRow('Price', `<strong style="color:#059669;">${productPrice}</strong>`) : '') +
              `</table>`,
              '#1a72f0'
            )
          : '') +
        (specialOffer
          ? callout(
              `<p style="margin:0; font-family:'Inter',sans-serif; font-size:14px; font-weight:600; color:#374151;">` +
              `<span style="color:#db2777;">Special Offer:</span> ${specialOffer}</p>`,
              '#db2777'
            )
          : '') +
        extra +
        paragraph(`Stay tuned for more great deals and product drops. Happy shopping!`)
      break
    }
    default:
