...
export default function ManageBookingsPage() {
  ...
  const loadBookings = async () => {
    try {
      ...
      const res = await fetch('/api/admin/photography-bookings', { headers })
  ...
  const updateBookingStatus = async (bookingId: string, newStatus: string) => {
    setSavingStatusForId(bookingId)
    try {
      const headers = await getAuthHeaders()
  ...
  const updateAdminNotes = async (bookingId: string) => {
    try {
      ...
      const res = await fetch('/api/admin/photography-bookings', {
        method: 'PATCH',
        headers,
        body: JSON.stringify({ id: bookingId, admin_notes: adminNotes }),
      })
      const data = await res.json()
  ...
  const deleteBooking = async (bookingId: string) => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/admin/photography-bookings?id=${encodeURIComponent(bookingId)}`,
                ...
                <div className="flex gap-2">
                  {(['pending', 'confirmed', 'cancelled', 'completed'] as const).map((status) => (
                    <button
                      key={status}
                      type="button"
                      disabled={savingStatusForId === selectedBooking.id}
