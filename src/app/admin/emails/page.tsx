...
function IndividualPanel({ templateId }: { templateId: TemplateId }) {
  ...
  // reset + fetch student list whenever template changes
  useEffect(() => {
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="p-6 sm:p-8 space-y-6">
          <p className="text-xs font-black uppercase tracking-widest text-gray-400">Step 2 — Review &amp; edit message</p>
...
function BulkPanel() {
  ...
  const previewHtml = useMemo(() => {
    ...
    const subj = subject.trim() || 'Notice from Purpose Technology'
    const paragraphs = bodyText
      .split(/\n{2,}/)
      .map((block) =>
        `<p style="margin:0 0 16px 0;">${block.trim().replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\n/g, '<br />')}</p>`
      ).join('\n')

    const ctaBlock = ctaLabel.trim() && ctaHref.trim()
      ? `<tr><td style="padding:12px 40px 0; text-align:center;">
          <table role="presentation" cellspacing="0" cellpadding="0" border="0" style="margin:0 auto;">
            <tr><td style="border-radius:14px; background-color:#1a72f0;">
              <a href="${ctaHref.trim()}" target="_blank" style="display:inline-block;padding:18px 36px;font-family:-apple-system,sans-serif;font-size:14px;font-weight:700;color:#fff;text-decoration:none;border-radius:14px;text-transform:uppercase;letter-spacing:0.1em;">${ctaLabel.trim()}</a>
            </td></tr>
          </table>
        </td></tr>`
      : ''

    return `<!DOCTYPE html><html><head><meta charset="utf-8"><style>body{margin:0;padding:0;background:#eff3f8}table,td{mso-table-lspace:0pt;mso-table-rspace:0pt}</style></head>
<body style="margin:0;padding:0;background:#eff3f8;">
<table width="100%" cellspacing="0" cellpadding="0" border="0" style="background:#eff3f8;padding:40px 20px;"><tr><td align="center">
<table width="100%" cellspacing="0" cellpadding="0" border="0" style="max-width:600px;background:#fff;border-radius:32px;overflow:hidden;box-shadow:0 10px 40px rgba(0,0,0,.05);">
  <tr><td style="background:linear-gradient(135deg,#020b1a 0%,#003580 100%);padding:48px 40px;text-align:center;">
    <div style="margin-bottom:16px;"><span style="font-family:-apple-system,sans-serif;font-size:11px;font-weight:800;color:#5a9ef5;letter-spacing:.4em;text-transform:uppercase;">Purpose Technology</span></div>
    <h1 style="margin:0;font-family:-apple-system,sans-serif;font-size:28px;font-weight:700;color:#fff;line-height:1.2;">${subject.trim() || 'Notice from Purpose Technology'}</h1>
  </td></tr>
  <tr><td style="height:4px;background:linear-gradient(90deg,#1a72f0 0%,#003580 100%);font-size:0;line-height:0;">&nbsp;</td></tr>
  <tr><td style="padding:48px 40px 0;font-family:-apple-system,sans-serif;font-size:15px;line-height:1.6;color:#1f2937;">${paragraphs}</td></tr>
  ${ctaBlock}
  <tr><td style="padding:48px 40px 40px;border-top:1px solid #f3f4f6;margin-top:40px;">
    <p style="margin:0;font-size:11px;font-weight:600;color:#9ca3af;text-transform:uppercase;letter-spacing:.05em;">© ${new Date().getFullYear()} Purpose Technology Namibia</p>
  </td></tr>
