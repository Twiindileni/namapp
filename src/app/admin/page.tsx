...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    ...
    const fetchStats = async (showLoadingToast = false) => {
      const controller = new AbortController()
      const timeout = setTimeout(() => controller.abort(), 15000)
      try {
        if (showLoadingToast) {
          loadingToastId = toast.loading('Refreshing dashboard...')
        }
        ...
        toast.error('Failed to load dashboard stats (network/cache issue)')
      } finally {
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-8">
            ...
            <div className="mt-4 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <Link href="/admin/emails" className="relative block w-full rounded-lg border-2 border-dashed border-violet-300 p-12 text-center hover:border-violet-400 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-offset-2 bg-violet-50 hover:bg-violet-100 transition-all">
                <svg className="mx-auto h-12 w-12 text-violet-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                </svg>
                <span className="mt-2 block text-sm font-semibold text-violet-900">Email customers (Resend)</span>
              </Link>

              <Link href="/admin/photography" className="relative block w-full rounded-lg border-2 border-dashed border-indigo-300 p-12 text-center hover:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 bg-indigo-50 hover:bg-indigo-100 transition-all">
